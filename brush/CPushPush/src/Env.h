...
namespace push {
        ...
        public:
            ...
            virtual unsigned get_stack_size(int which) const {
                switch (which) {
    				...
    				case STRING_STACK: return string_stack.size();
                }
                return 0;
            }
            ...
            void add_input_data(Data* d) {
                if (d != NULL) {
                    this->inputSampleData = d;
                    d->makeInputInstructions(); // Register functions with code base
                }
                
            }
            int next_sample() {
                return this->inputSampleData->next_sample();
            }
    ...
    template <> inline std::vector<std::string>& get_stack(Env& env)	{ return env.string_stack; }

    template <typename T> inline std::vector<T>& get_input_stack(Env& env)    { return get_input_stack<T>(env); }
    template <> inline std::vector<Exec>&        get_input_stack(Env& env)    { return env.inputSampleData->getCurExec(); }
    template <> inline std::vector<int>&         get_input_stack(Env& env)    { return env.inputSampleData->getCurInt();  }
    template <> inline std::vector<double>&      get_input_stack(Env& env)    { return env.inputSampleData->getCurDouble(); }
    template <> inline std::vector<bool>&        get_input_stack(Env& env)    { return env.inputSampleData->getCurBool(); }
    template <> inline std::vector<name_t>&      get_input_stack(Env& env)    { return env.inputSampleData->getCurName(); }
    template <> inline std::vector<Code>&        get_input_stack(Env& env)    { return env.inputSampleData->getCurCode(); }
    template <> inline std::vector<std::string>& get_input_stack(Env& env)    { return env.inputSampleData->getCurStringInputs(); }
    ...
    // Should there be capacity check?
